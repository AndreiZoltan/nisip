...
def charge_graph(x: int, y: int, shape: tuple) -> np.ndarray:
    ...
    graph[:x+1, y:-1] |= 1<<0
    graph[1:x+1, y:] |= 1<<3
    graph[:x, y+1:-1] |= np.where(grid[0] + grid[1] >= x+y, 1<<4, 0)[:x, y+1:-1]
    graph[1:x, y+1:] |= np.where(grid[0] + grid[1] <= x+y, 1<<5, 0)[1:x, y+1:]

    graph[x:, 1:y+1] |= 1<<1
    graph[x:-1, :y+1] |= 1<<2
    graph[x+1:-1, :y] |= np.where(grid[0] + grid[1] >= x+y, 1<<4, 0)[x+1:-1, :y]
    ...
    graph[1:x+1, :y+1] |= np.where(grid[1] >= grid[0] + y - x, 1<<3, 0)[1:x+1, :y+1]
    graph[1:x+1, 1:y+1] |= np.where(grid[1] >= grid[0] + y - x, 1<<5, 0)[1:x+1, 1:y+1]
    graph[:x, :y] |= np.where(grid[1] >= grid[0]/2 - x/2 + y, 1<<0, 0)[:x, :y]
    graph[:x, 1:y] |= np.where((grid[1] <= grid[0]/2 - x/2 + y) & (grid[1] > grid[0] + y - x), 1<<1, 0)[:x, 1:y]
    
    graph[x:, y:-1] |= np.where(grid[1] >= grid[0] + y - x, 1<<0, 0)[x:, y:-1]
    graph[x:-1, y:-1] |= np.where(grid[1] >= grid[0] + y - x, 1<<4, 0)[x:-1, y:-1]
    graph[x+1:, y+1:] |= np.where(grid[1] >= 2*grid[0] - 2*x + y, 1<<3, 0)[x+1:, y+1:]
    graph[x+1:-1, y+1:] |= np.where((grid[1] <= 2*grid[0] - 2*x + y) & (grid[1] > grid[0] + y - x), 1<<2, 0)[x+1:-1, y+1:]
    
    graph[x:-1, y:] |= np.where(grid[1] <= grid[0] + y - x, 1<<2, 0)[x:-1, y:]
    graph[x:-1, y:-1] |= np.where(grid[1] <= grid[0] + y - x, 1<<4, 0)[x:-1, y:-1]
    graph[x+1:, y+1:] |= np.where(grid[1] <= grid[0]/2 - x/2 + y, 1<<1, 0)[x+1:, y+1:]
    graph[x+1:, y+1:-1] |= np.where((grid[1] >= grid[0]/2 - x/2 + y) & (grid[1] < grid[0] + y - x), 1<<0, 0)[x+1:, y+1:-1]
    return graph