...
static PyObject *relax_triangular_directed_irregular_non_trivial_boundary(PyObject *self, PyObject *args)
{
    PyArrayObject *graph, *directions, *degrees, *boundary;
    PyArg_ParseTuple(args, "OOOO", &graph, &directions, &degrees, &boundary);
    if (PyErr_Occurred())
    {
        return NULL;
    }
    if (!PyArray_Check(graph) || !PyArray_Check(directions) || !PyArray_Check(degrees) || !PyArray_Check(boundary))
    {
        PyErr_SetString(PyExc_TypeError, "Argument must be numpy array.");
        return NULL;
    }
    ...
    long long **graph_data, **directions_data, **degrees_data, **boundary_data;
    int64_t size = PyArray_SIZE(graph);
    npy_intp *dims = PyArray_DIMS(graph);
    graph = PyArray_Cast(graph, NPY_INT64);
    PyArray_AsCArray((PyObject **)&graph, &graph_data, dims, 2, PyArray_DescrFromType(NPY_INT64));
    ...
    for (int i = 0; i < (int)dims[0]; i++)
    {
        for(int j = 0; j < (int)dims[1]; j++)
        {
            ...
            ptr[i*2*dims[1] + 2*j + 1] = directions_data[i][j];
            ptr[i*2*dims[1] + 2*j + 1] += degrees_data[i][j]<<6;
            ptr[i*2*dims[1] + 2*j + 1] += boundary_data[i][j]<<12;
        }
    }
    relax_triangular_directed_irregular_with_boundary(ptr, (int)dims[0], (int)dims[1]);
    PyObject *result = PyArray_SimpleNew(2, dims, NPY_INT64);
    if (!PyArray_IS_C_CONTIGUOUS(result))
    {
        printf("Python created not contiguous array\n");
...
static PyMethodDef methods[] = {
    ...
    {"relax_triangular_directed_irregular_non_trivial_boundary", relax_triangular_directed_irregular_non_trivial_boundary, METH_VARARGS, "Relax sandpile on triangular lattice graph with directed edges and non-trivial boundary"},
    {NULL, NULL, 0, NULL}};
